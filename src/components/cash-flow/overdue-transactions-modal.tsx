...
export function OverdueTransactionsModal({ open, onOpenChange, onUpdate, initialTab = 'expenses' }: OverdueTransactionsModalProps) {
  ...
  // Filter overdue credit card transactions
  const overdueCreditCardTransactions = purchaseOrders.filter(tx => {
    const dueDate = new Date(tx.dueDate);
    dueDate.setHours(0, 0, 0, 0);
  ...
  const handleMarkCreditCardPaid = async (id: string) => {
    setProcessingId(id);
    try {
      await markPOPaid(id);
      onUpdate?.();
  ...
  const handleChangeCreditCardDate = async (tx: VendorTransaction, newDate: Date) => {
    setProcessingId(tx.id);
    try {
      await updatePODate(tx.id, newDate);
      setChangingDateId(null);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
          ...
          {/* Credit Card Purchases Tab */}
          <TabsContent value="creditCard" className="space-y-4">
            {overdueCreditCardTransactions.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">No overdue credit card purchases</p>
            ) : (
              <div className="space-y-3">
                {overdueCreditCardTransactions.map((tx) => (
                  <div key={tx.id} className="border rounded-lg p-4 bg-destructive/5">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="font-medium">{tx.vendorName}</div>
                        <div className="text-sm text-muted-foreground">{tx.category || 'Credit Card Purchase'}</div>
                        {tx.remarks && <div className="text-sm text-muted-foreground mt-1">{tx.remarks}</div>}
                        <div className="text-sm text-destructive mt-1">
                          Due: {formatDate(new Date(tx.dueDate))}
                        </div>
                        <div className="text-lg font-bold mt-2">{formatCurrency(tx.amount)}</div>
                      ...
                      <div className="flex gap-2">
                        <Popover open={changingDateId === tx.id} onOpenChange={(open) => !open && setChangingDateId(null)}>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <CalendarComponent
                              mode="single"
                              selected={new Date(tx.dueDate)}
                              onSelect={(date) => date && handleChangeCreditCardDate(tx, date)}
                              initialFocus
                              className={cn("p-3 pointer-events-auto")}
                            />
                          </PopoverContent>
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleMarkCreditCardPaid(tx.id)}
                          disabled={processingId === tx.id}
                        >
                          <CheckCircle className="h-4 w-4 mr-1" />
                          Mark Paid
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => openDeleteConfirm(tx.id, 'creditCard', tx.vendorName)}
                          disabled={processingId === tx.id}
                          className="text-destructive hover:text-destructive"
                        >
                          <Trash2 className="h-4 w-4" />
