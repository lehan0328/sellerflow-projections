  ...
  return <>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4" style={{
          ...
          {/* Mathematical Forecasting Model Selection - Show per account */}
          {amazonAccounts && amazonAccounts.length > 0 ? (
            <div className="space-y-3">
              {amazonAccounts.map((account) => {
                const accountPayoutModel = account.payout_frequency || 'bi-weekly';
                
                return (
                  <div key={account.id} className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                          <div className="p-2 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
                            <p className="text-xs text-green-800 dark:text-green-200 flex items-center gap-1.5">
                              ...
                              Auto-detected from settlements: <span className="font-semibold">{accountPayoutModel === 'bi-weekly' ? '14-Day Settlements (Bi-weekly)' : 'Daily Settlements (<14 days apart)'}</span>
                            </p>
                          </div>
                          
                          <div>
                            <Label className="text-xs font-medium text-blue-900 dark:text-blue-100 mb-2 block">
                              Payout Model (Auto-detected)
                            </Label>
                            <div className="grid grid-cols-2 gap-2">
                              <div className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${accountPayoutModel === 'bi-weekly' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 opacity-50'}`}>
                                <div className="font-semibold">14-Day Settlements</div>
                                <div className="text-[10px] opacity-80">Bi-weekly payouts</div>
                              </div>
                              <div className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${accountPayoutModel === 'daily' ? 'bg-purple-600 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 opacity-50'}`}>
                                <div className="font-semibold">Daily Settlements</div>
                                <div className="text-[10px] opacity-80">&lt;14 day intervals</div>
                              </div>
                            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-blue-800 dark:text-blue-200">
                    Connect an Amazon account to enable payout forecasting
                  </p>
                </div>
              </div>
            </div>
