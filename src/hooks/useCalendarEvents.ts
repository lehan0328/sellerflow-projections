...
export const useCalendarEvents = (): CalendarEventsResult => {
  ...
  const calendarEvents = useMemo(() => {
    ...
    const recurringEvents: CalendarEvent[] = recurringExpenses
      ...
      .flatMap(expense => {
        ...
        const dates = generateRecurringDates(transactionObj, today, addDays(today, 365));
        return dates.map((date): CalendarEvent => ({
